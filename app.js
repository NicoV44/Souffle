(() => {
  ...
  const state = {
    ...
    microphoneStatus: 'off', voiceEnabled: false, leadInTimer: 0
  };
  const cueAudio = {
    inhale: new Audio('./audio/inspiration.wav'),
    exhale: new Audio('./audio/expiration.wav')
  };
  ...
  function startSession() {
    ...
    state.voiceEnabled = $('voice-enabled').checked;
    showPhase('breathing');
    state.phase = 'leadIn';
    ...
    speak('Cycle un. Installe-toi. La première inspiration arrive.', { cancel: true });
    requestWakeLock();
    cancelAnimationFrame(state.raf);
  ...
  $('voice-enabled').addEventListener('change', event => { state.voiceEnabled = event.target.checked; localStorage.setItem('souffle-voice-enabled-v1', event.target.checked ? 'yes' : 'no'); });
  ...
  state.voiceEnabled = localStorage.getItem('souffle-voice-enabled-v1') === 'yes';
